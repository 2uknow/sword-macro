...
class SwordAI:
    ...
    def _get_mask(self, fund, level, mode = 'ai'):
        ...
        # 목표 레벨 달성 시 강화/판매 모두 중단
        if level >= MAX_LEVEL_FOR_ENHANCE:
            can_enhance = False
            can_sell = False
    ...
    def heuristic(self, fund: int, level: int, fail_count = 0):
        ...
        if mask[0] and not mask[1]:
            ...
            # FORCE_SELL_LEVEL 이상이면 무조건 판매
            if level >= FORCE_SELL_LEVEL:
                return 1  # 무조건 판매
            elif level < LEVEL_THRESHOLD or fail_count < FAIL_COUNT_THRESHOLD:
                return 0  # 강화
            else:
                return 1  # 판매
        else:
